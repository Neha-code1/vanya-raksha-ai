import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import './Header.css';

const Header = () => {
  const { user, loading, error, signInWithGoogle, logOut, clearError } = useAuth();
  const [signingIn, setSigningIn] = useState(false);

  const handleGoogleSignIn = async () => {
    setSigningIn(true);
    try {
      await signInWithGoogle();
    } catch {
      // error is already set in AuthContext
    } finally {
      setSigningIn(false);
    }
  };

  return (
    <>
      <header className="header glass">
        <div className="container header-content">

          {/* ── Logo (unchanged) ── */}
          <Link to="/" className="logo-section" style={{ textDecoration: 'none' }}>
            <div className="logo-icon">🌿</div>
            <h1 className="logo-text">Vanya Raksha AI</h1>
          </Link>

          {/* ── Auth Buttons ── */}
          <div className="auth-buttons">

            {/* While Firebase is checking stored session on load */}
            {loading ? (
              <div className="auth-loading">
                <span className="loading-dot" />
                <span className="loading-dot" />
                <span className="loading-dot" />
              </div>

            ) : user ? (
              /* ── LOGGED IN: show user info + logout ── */
              <div className="user-info">
                <img
                  src={user.photoURL || '/default-avatar.png'}
                  alt={user.displayName || 'User'}
                  className="user-avatar"
                  referrerPolicy="no-referrer"
                />
                <div className="user-text">
                  <span className="user-name">{user.displayName}</span>
                  <span className="user-email">{user.email}</span>
                </div>
                <button
                  className="btn btn-outline logout-btn"
                  onClick={logOut}
                >
                  Sign Out
                </button>
              </div>

            ) : (
              /* ── LOGGED OUT: show sign-in button ── */
              <button
                className="btn btn-primary google-btn"
                onClick={handleGoogleSignIn}
                disabled={signingIn}
              >
                {signingIn ? (
                  <>
                    <span className="spinner" />
                    Signing in...
                  </>
                ) : (
                  <>
                    <GoogleIcon />
                    Sign in with Google
                  </>
                )}
              </button>
            )}
          </div>
        </div>
      </header>

      {/* ── Error Toast (shown below header on auth failure) ── */}
      {error && (
        <div className="auth-error-toast" role="alert">
          <span>⚠️ {error}</span>
          <button className="toast-close" onClick={clearError} aria-label="Dismiss">✕</button>
        </div>
      )}
    </>
  );
};

// ── Google SVG Icon (extracted for cleanliness) ──────────────────
const GoogleIcon = () => (
  <svg className="google-icon" viewBox="0 0 24 24" aria-hidden="true" width="18" height="18">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </svg>
);

export default Header;

